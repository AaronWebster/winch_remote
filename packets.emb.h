...
class EnumTraits<Direction> final {
 public:
  static bool TryToGetEnumFromName(const char *emboss_reserved_local_name,
    ...
    // repeated strcmp().
    if (!strcmp("UNKNOWN", emboss_reserved_local_name)) {
      *emboss_reserved_local_result = Direction::UNKNOWN;
      return true;
    }
    ...
    if (!strcmp("STOP", emboss_reserved_local_name)) {
      *emboss_reserved_local_result = Direction::STOP;
      return true;
    }
...
class GenericPacketView final {
 public:
  ...
  bool Ok() const {
    ...
    // If we don't have enough information to determine whether sequence_id() is
    // present in the structure, then structure.Ok() should be false.
    if (!has_sequence_id().Known()) return false;
    // If sequence_id() is present, but not Ok(), then structure.Ok() should be
    // false.  If sequence_id() is not present, it does not matter whether it is
    // Ok().
    if (has_sequence_id().ValueOrDefault() && !sequence_id().Ok()) return false;
  ...
  bool Equals(
      GenericPacketView<OtherStorage> emboss_reserved_local_other) const {
        // If this->sequence_id() is not equal to emboss_reserved_local_other.sequence_id(),
    // then the structures are not equal.

    // If either structure's has_sequence_id() is unknown, then default to not
    // Equals().
    //
    // TODO(bolms): Should Equals() return Maybe<bool> and/or return true for
    // non-Ok()-but-equivalent structures?
    if (!has_sequence_id().Known()) return false;
    if (!emboss_reserved_local_other.has_sequence_id().Known()) return false;

    // If one side has sequence_id() but the other side does not, then the fields
    ...
    // has_sequence_id().Known() is true for both structures.
    if (emboss_reserved_local_other.has_sequence_id().ValueOrDefault() &&
        !has_sequence_id().ValueOrDefault())
      return false;
    if (has_sequence_id().ValueOrDefault() &&
        !emboss_reserved_local_other.has_sequence_id().ValueOrDefault())
      return false;

    // If both sides have sequence_id(), then check that their Equals() returns
    // true.
    if (emboss_reserved_local_other.has_sequence_id().ValueOrDefault() &&
        has_sequence_id().ValueOrDefault() &&
        !sequence_id().Equals(emboss_reserved_local_other.sequence_id()))
      return false;
 ...
 return true;
  ...
  bool UncheckedEquals(
    ...
    // both views are Ok() (which implies that has_sequence_id().Known() is true),
    // and UncheckedEquals() will never perform any assertion checks (which
    // implies that UncheckedEquals() cannot call has_sequence_id().Value()).

    // If this->has_sequence_id() but !emboss_reserved_local_other.has_sequence_id(), or
    // vice versa, then the structures are not equal.  If neither structure
    // has_sequence_id(), then sequence_id() is considered equal.
    if (emboss_reserved_local_other.has_sequence_id().ValueOr(false) &&
        !has_sequence_id().ValueOr(false))
      return false;
    if (has_sequence_id().ValueOr(false) &&
        !emboss_reserved_local_other.has_sequence_id().ValueOr(false))
      return false;

    // If sequence_id() is present in both structures, then check its equality.
    if (emboss_reserved_local_other.has_sequence_id().ValueOr(false) &&
        has_sequence_id().ValueOr(false) &&
        !sequence_id().UncheckedEquals(emboss_reserved_local_other.sequence_id()))
      return false;
 ...
 return true;
  ...
  bool UpdateFromTextStream(Stream *emboss_reserved_local_stream) const {
    ...
    for (;;) {
      ...
      // If the field name matches sequence_id, handle it, otherwise fall
      // through to the next field.
      if (emboss_reserved_local_name == "sequence_id") {
        // TODO(bolms): How should missing optional fields be handled?
        if (!sequence_id().UpdateFromTextStream(
  ...
  void WriteToTextStream(
    ...
    if (has_sequence_id().ValueOr(false)) {
      ...
      if (!emboss_reserved_local_field_options.allow_partial_output() ||
          sequence_id().IsAggregate() || sequence_id().Ok()) {
        ...
        emboss_reserved_local_stream->Write("sequence_id: ");
        sequence_id().WriteToTextStream(emboss_reserved_local_stream,
        ...
        emboss_reserved_local_stream->Write("# sequence_id: UNREADABLE\n");
      }
    }
 ...
 sequence_id() const;
  ::emboss::support::Maybe<bool> has_sequence_id() const;

 public:
  typename ::emboss::support::EnumView<
    /**/ ::internal::Direction,
    ::emboss::support::FixedSizeViewParameters<8, ::emboss::support::AllValuesAreOk>,
    typename ::emboss::support::BitBlock</**/::emboss::support::BigEndianByteOrderer<typename Storage::template OffsetStorageType</**/0, 4>>, 8>>
...
inline typename ::emboss::prelude::UIntView<
 ...
 GenericPacketView<Storage>::sequence_id()
    const {
...
GenericPacketView<Storage>::has_sequence_id() const {
  return ::emboss::support::Maybe</**/bool>(true);
}
